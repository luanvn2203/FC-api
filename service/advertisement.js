...
async function getAllAdvertiseByEmail(email) {
    try {
        ...
        const sql = `SELECT a.id, a.title, a.content, a.imageLink, a.startDate, a.endDate, a.donorId, a.statusId, a.target_url , a.expected_using_point ,a.time_rendering , ass.status  as statusName
...
async function getRunningAdsWithExpiredEndDate(advertiseId) {
    try {
        ...
        expected_using_point,time_rendering from tbl_advertisement where id = ? and statusId != 4 
        and endDate < current_timestamp`;
        const params = [
            `${advertiseId}`,
        ]
